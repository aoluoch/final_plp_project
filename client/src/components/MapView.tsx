...
import type { Map as MapboxMap, Marker as MapboxMarker, MapMouseEvent } from 'mapbox-gl'
...
const MapView: React.FC<MapViewProps> = ({
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      if (mapInstanceRef.current) {
        ...
        const targetZoom = typeof feature.properties?.['mapbox:zoom'] === 'number'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const mod = (await import('mapbox-gl')) as unknown as { default: typeof import('mapbox-gl') }
  ...
  useEffect(() => {
    ...
    const existing = (map as unknown as Record<string, MapboxMarker[] | undefined>)[registryKey] || []
    existing.forEach((m) => m.remove())

    const addMarker = async (m: Required<Required<MapViewProps>['markers']>[number] & { color: string }) => {
      const mod = (await import('mapbox-gl')) as unknown as { default: typeof import('mapbox-gl') }
      const mapbox = mod.default
  ...
  return (
    ...
    >
      ...
      {mapReady && (
        <div className="absolute z-10 top-2 left-2 right-2 md:right-auto md:w-96" onClick={(e) => e.stopPropagation()}>
