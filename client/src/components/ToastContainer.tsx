...
const ToastContainer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    toasts.forEach((t) => {
      if (!leaving[t.id]) {
        const timer = window.setTimeout(() => handleDismiss(t.id), t.duration || 4000)
        timers.push(timer)
      }
    })
    return () => {
      ...
      <div className="space-y-3">
        {toasts.map((t) => (
          ...
          >
            ...
            <button aria-label="Dismiss" onClick={() => handleDismiss(t.id)} className="ml-3 opacity-80 hover:opacity-100">
              ×
            </button>
          </div>
        ))}
