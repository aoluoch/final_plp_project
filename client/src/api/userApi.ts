...
export const userApi = {
  ...
  async activateUser(id: string): Promise<User> {
    const response = await axiosInstance.post<ApiResponse<User>>(`/users/${id}/activate`)
    return response.data.data
  },

  async deactivateUser(id: string): Promise<User> {
    const response = await axiosInstance.post<ApiResponse<User>>(`/users/${id}/deactivate`)
    return response.data.data
  },

  async getCollectors(): Promise<User[]> {
    const response = await axiosInstance.get<ApiResponse<User[]>>('/users/collectors')
    return response.data.data
  },

  async getResidents(): Promise<User[]> {
    const response = await axiosInstance.get<ApiResponse<User[]>>('/users/residents')
    return response.data.data
  },
}
