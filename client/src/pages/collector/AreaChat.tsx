...
const AreaChat: React.FC = () => {
  ...
  const areaRoom = useMemo(() => {
    const lat = (user as { address?: { coordinates?: { lat?: number; lng?: number } } } | null)?.address?.coordinates?.lat
    const lng = (user as { address?: { coordinates?: { lat?: number; lng?: number } } } | null)?.address?.coordinates?.lng
  ...
  useEffect(() => {
    ...
    const handleNew = (...args: unknown[]) => {
      ...
      if (msg?.room === areaRoom) {
        ...
        const normalizedSenderName = msg.sender?.name || msg.senderName || 'System'
        const normalizedSenderRole = msg.sender?.role || msg.senderRole || 'system'
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    socket.emit('send_message', { room: areaRoom, message: newMessage.trim(), type: 'text' })
    setMessages(prev => [...prev, {
      id: `tmp-${Date.now()}`,
      senderId: user.id,
  ...
  return (
    <div className="h-[calc(100vh-12rem)] flex flex-col">
      <div className="bg-white dark:bg-gray-800 rounded-t-lg shadow p-4 border-b border-gray-200 dark:border-gray-700">
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">Area Chat</h1>
        <p className="text-sm text-gray-600 dark:text-gray-400">Chat with collectors in your area</p>
      </div>
      <div className="flex-1 bg-white dark:bg-gray-800 overflow-y-auto p-4 space-y-4">
        {messages.map(m => (
          <div key={m.id} className={`flex ${m.senderId === user?.id ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-xs lg:max-w-md ${m.senderId === user?.id ? 'bg-blue-600 text-white' : 'bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white'} rounded-lg px-4 py-2`}>
              {m.senderId !== user?.id && (<div className="text-xs font-medium mb-1 opacity-75">{m.senderName}</div>)}
              <div className="text-sm">{m.message}</div>
              <div className={`text-xs mt-1 ${m.senderId === user?.id ? 'text-blue-100' : 'text-gray-500 dark:text-gray-400'}`}>
