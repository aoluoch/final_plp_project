...
const RouteOptimization: React.FC = () => {
  ...
  const { data: tasksData } = useFetch(
    () => pickupApi.getMyTasks(1, 50),
    { immediate: true }
  )
  ...
  const todayTasks = tasks.filter(task => {
    ...
    return taskDate === selectedDate && task.status !== 'completed' && task.status !== 'cancelled'
  })
  ...
  const optimizeRoute = async () => {
    ...
    try {
      ...
      const priorityDiff = priorityOrder[bPriority as keyof typeof priorityOrder] - 
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600 dark:text-red-400">
                {optimizedRoute.filter(t => t.report?.priority === 'high').length}
              </div>
